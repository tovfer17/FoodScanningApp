...
const listStoresFromGoogle = async (name, long, lat, radius=2000) => {
    ...
    let res = await fetch(`https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input=${encoded_name}&inputtype=textquery&fields=photos,formatted_address,name,opening_hours&locationbias=circle:${radius}@${lat},${long}&key=${process.env.GOOGLE_API_KEY}`)
